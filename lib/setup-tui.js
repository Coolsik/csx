...
import { RESERVED_PRESET_NAMES, ROLE_NAMES, cloneMatrix } from "./presets.js";
import { escapeTerminalText } from "./terminal-text.js";

const h = React.createElement;
const RESERVED_NAMES = new Set(RESERVED_PRESET_NAMES);
const REVIEW_ACTIONS = ["Apply", "Cancel"];
const ALL_ROLES = "all";

const ROLE_PRESENTATION = Object.freeze({
  leader: { tag: "LEADER", label: "Leader", color: "white" },
...
export function matrixMatches(left, right) {
  try {
    ...
    return ROLE_NAMES.every((name) => samePair(a[name], b[name]));
  } catch {
    return false;
  }
}
...
export function invalidRows(matrix, catalog) {
  ...
  return ROLE_NAMES.filter((name) => !available.get(matrix[name]?.model)?.has(matrix[name]?.reasoning));
}

export function changedRows(baselineMatrix, matrix) {
  return ROLE_NAMES.filter((name) => !samePair(baselineMatrix[name], matrix[name]));
}
...
function itemsForScreen(state) {
  ...
  if (state.screen === "assign-role") {
    return [
      ...ROLE_NAMES.map((agent) => ({ kind: "role", id: agent, agent })),
...
function itemLines(state, item, selected) {
  ...
  if (state.screen === "models") {
    ...
    if (selected) {
      lines.push(...ROLE_NAMES.map((agent) =>
    ...
    if (item.kind === "change") {
      ...
      lines = [`${prefix}No role model changes.`];
...
function roleDefaultEffort(state, target) {
  ...
  if (target === ALL_ROLES) {
    const first = state.draftMatrix[ROLE_NAMES[0]];
    if (
      ROLE_NAMES.every((agent) => samePair(state.draftMatrix[agent], first)) &&
...
function applyEffort(state) {
  ...
  const agents = state.pendingTarget === ALL_ROLES ? ROLE_NAMES : [state.pendingTarget];
