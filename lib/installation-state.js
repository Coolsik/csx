...
/** Describes a receipt-owned target against an independently derived exact path set. */
export async function existingInstallationTarget({ root, configPath, receiptPath, expectedFiles } = {}) {
  ...
  return Object.freeze({ role: "existing-installation-target", root: resolve(root), configPath: resolve(configPath), receiptPath: resolve(receiptPath), receipt, receiptSnapshot: { state: "present", data: receiptSnapshot.data.toString("base64"), hash: digest(receiptSnapshot.data), mode: receiptSnapshot.mode }, paths: [...owned].sort() });
}
