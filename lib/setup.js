...
import {
  install,
  normalizeReceiptSetupMatrix,
  projectMigrationPreflight,
  receiptOwnedPaths,
  setupReceiptMatrix
} from "./install.js";
import {
...
export async function selectSetupScope(options = {}) {
  const preflight = await (options.projectMigrationPreflightFn ?? projectMigrationPreflight)(options);
  if (preflight.historicalCount) {
    await (options.installFn ?? install)({
      scope: "project",
      cwd: options.cwd,
      env: options.env
    });
  }
  const { project, global } = setupLayout({ ...options, cwd: preflight.root });
...
export async function readSetupMatrix(layout) {
  return cloneMatrix({
    leader: await readWorkflowLeader(layout.agentsRoot),
    ...await readAgentMatrix(layout.agentsRoot)
  });
}

async function readWorkflowLeader(agentsRoot) {
  const pairs = await Promise.all(WORKFLOW_LEADER_NAMES.map(async (name) =>
    parseAgent(await readFile(join(agentsRoot, `${name}.toml`), "utf8"), name)
  ));
  if (pairs.some(({ model, reasoning }) =>
    model !== pairs[0].model || reasoning !== pairs[0].reasoning
  )) {
    throw new Error("workflow Leader model configurations do not match.");
  }
  return pairs[0];
}
...
export async function readCustomPresets({ env = process.env } = {}) {
  ...
  for (const [name, matrix] of Object.entries(parsed.presets)) {
    ...
    if (!name.trim() || names.has(normalized)) throw new Error(`invalid custom preset file: ${path}`);
    names.add(normalized);
    try {
      presets[name] = parsed.version === 1 ? upgradeLegacyMatrix(matrix) : cloneMatrix(matrix);
    } catch {
      throw new Error(`invalid custom preset file: ${path}`);
    }
  }
  return { path, hash: digest(text), presets };
}

export async function applySetup({ layout, cwd = process.cwd(), matrix, baselineMatrix, catalog, catalogLoader = async () => catalog, customPresetName, selectedAgents = ROLE_NAMES, env = process.env, transactionFactory = beginTransaction, expectedFilesLoader = receiptOwnedPaths } = {}) {
  ...
  if (!Array.isArray(selectedAgents) || selected.size !== selectedAgents.length || [...selected].some((name) => !ROLE_NAMES.includes(name))) {
    throw new Error("selected setup roles are invalid.");
  }
  if (!layout) layout = await selectSetupScope({ cwd, env });
  ...
  let target;
  try {
    target = await existingInstallationTarget({
      root: layout.root,
    ...
    if (/receipt does not match the installed package paths/.test(error.message)) {
      throw new Error("legacy csx installation detected; run `csx install` before `csx setup`.");
    }
    throw error;
  }
  try {
    normalizeReceiptSetupMatrix(target.receipt.setupAgentMatrix);
  } catch {
    throw new Error("legacy or invalid setup receipt detected; run `csx install` before `csx setup`.");
  }
  if (customPresetName !== undefined) {
    const global = setupLayout({ env }).global;
    ...
    metadataText = `${JSON.stringify({ version: 2, presets }, null, 2)}\n`;
    metadata = { ...(await metadataParticipant({ root: global.root, paths: [probe.path] })), coordinationRoot: setupCoordinationRoot(global) };
  }
  await Promise.all([
    ...selectedAgentPaths.map(async (path, index) =>
      scanTopLevelAssignments(await readFile(path, "utf8"), selectedAgentNames[index])
    ),
    ...(selected.has("leader") ? leaderPaths.map(async (path, index) =>
      scanTopLevelAssignments(await readFile(path, "utf8"), WORKFLOW_LEADER_NAMES[index])
    ) : [])
  ]);
  ...
  const receiptMatrix = setupReceiptMatrix(matrix);
  let changes;
  let plannedWrites;
  let receiptDrifted;
  const createDeclaration = async () => {
      ...
      const current = await readSetupMatrix(layout);
      if (previewMatrix !== undefined && !sameMatrix(current, previewMatrix)) {
        throw new Error("role matrix changed after preview; rerun setup.");
      }
      if (selected.size !== ROLE_NAMES.length && ROLE_NAMES.some((name) => !selected.has(name) && (current[name].model !== matrix[name].model || current[name].reasoning !== matrix[name].reasoning))) {
        throw new Error("unselected role settings changed after preview; rerun setup.");
      }
      ...
      if (
        selected.has("leader") &&
        (current.leader.model !== matrix.leader.model || current.leader.reasoning !== matrix.leader.reasoning)
      ) {
        changes.push(...leaderPaths);
      }
      if (metadata) {
      ...
      for (const path of changes) {
        ...
        const role = leaderPaths.includes(path) ? "leader" : AGENT_NAMES[agentPaths.indexOf(path)];
        const data = updateAgent(text, matrix[role]);
        plannedWrites.push({ path, data, mode: (await statMode(path)) });
      }
      if (metadata) plannedWrites.push({ path: probe.path, data: metadataText, mode: 0o600 });
      if (receiptDrifted) {
        const receipt = { ...target.receipt, setupAgentMatrix: receiptMatrix };
        plannedWrites.push({
          path: target.receiptPath,
          data: `${JSON.stringify(receipt, null, 2)}\n`,
          mode: await statMode(target.receiptPath)
        });
      }
      const writeSet = plannedWrites.map(({ path }) => path);
      return {
        operation: "setup",
        participants,
        snapshotSet,
        writeSet,
        finalEndpoints: Object.fromEntries(plannedWrites.map(({ path, data, mode }) => [
          path,
          {
            state: "present",
  ...
  let committed = false;
  try {
    for (const { path, data, mode } of plannedWrites.filter(({ path }) => changes.includes(path))) {
      await transaction.write(path, data, { mode });
    }
    if (changes.length) {
      const effective = await readSetupMatrix(layout);
      if (!sameMatrix(effective, receiptMatrix.roles)) throw new Error("role matrix did not match setup receipt after write.");
    }
    for (const { path, data, mode } of plannedWrites.filter(({ path }) => !changes.includes(path))) {
      await transaction.write(path, data, { mode });
    }
...
export async function builtInPresets() {
  return {
    Efficient: presetMatrix("Efficient"),
    Balanced: presetMatrix("Balanced"),
    Strong: presetMatrix("Strong")
  };
}
...
function sameMatrix(left, right) {
  return ROLE_NAMES.every((name) =>
    left[name]?.model === right[name]?.model &&
    left[name]?.reasoning === right[name]?.reasoning
  );
}
function sameSetupAgentMatrix(left, right) {
  if (!left || left.version !== 2 || !left.roles || typeof left.roles !== "object" || Array.isArray(left.roles)) return false;
  if (Object.keys(left).length !== 2 || Object.keys(left.roles).length !== ROLE_NAMES.length) return false;
  return ROLE_NAMES.every((name) =>
    left.roles[name]?.model === right.roles[name].model &&
    left.roles[name]?.reasoning === right.roles[name].reasoning
  );
}
