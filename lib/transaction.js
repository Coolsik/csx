...
export function recoveryAuthorityFromDeclaration({ coordinationRoots, participants, snapshotSet } = {}) {
  ...
  normalizedParticipants.forEach(validateParticipantDescriptor);
  ...
  if (typeof declaration?.createDeclaration === "function") {
    ...
    for (;;) {
      ...
      try {
        ...
        await runTransactionTestHook("beforeManifest", { operation, participants: stabilizedParticipants, snapshotSet, writeSet });
...
function validateDeclaration(operation, participants, snapshotSet, writeSet) {
  ...
  for (const participant of participants) {
    ...
    if (participant.role === "existing-installation-target" && participantAdditions(participant).length && operation !== "install") throw new Error("existing participant additions are valid only for install");
  }
  ...
  for (const participant of participants.filter((candidate) => candidate.role === "existing-installation-target" && participantAdditions(candidate).length)) {
    if (participant.paths.some((path) => !snapshot.has(resolve(path)))) throw new Error("existing transaction snapshotSet must exactly cover its authority");
    if (participantAdditions(participant).some((path) => !writes.has(path))) throw new Error("existing transaction additions must be included in writeSet");
  }
}
...
async function createManifest({ id, operation, participants, snapshotSet, writeSet, intended, roots, locks }) {
  const normalizedParticipants = participants.map(normalizeParticipant);
  const snapshots = {};
  for (const participant of normalizedParticipants.filter(({ role }) => role === "existing-installation-target")) {
    for (const path of participant.additions) {
      await assertSafeContainment(participant.root, path);
      await classifyLocalFilesystem(dirname(path));
      if ((await snapshot(path, undefined, locks)).state !== "absent") {
        throw new TransactionLockError("recovery_required", `existing transaction addition became present while locked: ${path}`);
      }
    }
  }
...
function validateManifest(manifest) {
  ...
  for (const participant of manifest.participants) {
    ...
    if (participant.role === "existing-installation-target" && participantAdditions(participant).some((path) => manifest.snapshots[path]?.state !== "absent")) throw new TransactionLockError("recovery_required", "existing transaction addition snapshot is invalid");
  }
}
...
async function assertRecoveryAuthority(manifest, locks, authority) {
  ...
  for (const participant of manifest.participants) {
    ...
    const expectedAuthority = new Set([...receiptPaths, resolve(participant.configPath), resolve(participant.receiptPath), ...participantAdditions(participant)]);
...
function normalizeParticipant(participant) {
  ...
  if (participant.role === "prospective-installation-target") {
    const split = prospectiveRecoverySplit(participant);
    return { ...base, configPath, receiptPath, preimages: structuredClone(participant.preimages), ...(split ?? {}) };
  }
  return { ...base, configPath, receiptPath, receipt: structuredClone(participant.receipt), receiptSnapshot: structuredClone(participant.receiptSnapshot), additions: participantAdditions(participant) };
}
function prospectiveRecoverySplit(participant) {
  const hasExpectedFiles = participant.expectedFiles !== undefined;
  const hasAdditions = participant.additions !== undefined;
  if (hasExpectedFiles !== hasAdditions) throw new Error("prospective recovery split requires expectedFiles and additions");
  if (!hasExpectedFiles) return null;
  if (!Array.isArray(participant.expectedFiles) || !Array.isArray(participant.additions)) throw new Error("prospective recovery split fields must be arrays");
  const normalize = (values, label) => {
    const paths = values.map((path) => {
      if (typeof path !== "string") throw new Error(`prospective recovery ${label} must contain paths`);
      return resolve(path);
    }).sort();
    if (new Set(paths).size !== paths.length) throw new Error(`prospective recovery ${label} must be unique`);
    return Object.freeze(paths);
  };
  return Object.freeze({ expectedFiles: normalize(participant.expectedFiles, "expectedFiles"), additions: normalize(participant.additions, "additions") });
}
function participantAdditions(participant) {
  const additions = participant.additions ?? [];
  if (!Array.isArray(additions)) throw new Error("existing participant additions must be an array");
  const normalized = additions.map((path) => {
    if (typeof path !== "string") throw new Error("existing participant additions must contain paths");
    return resolve(path);
  });
  if (new Set(normalized).size !== normalized.length) throw new Error("existing participant additions must be unique");
...
function validateParticipantDescriptor(participant) {
  ...
  if (participant.role === "prospective-installation-target") {
    ...
    if (split) {
      const root = resolve(participant.root);
      if ([...split.expectedFiles, ...split.additions].some((path) => !isContained(root, path))) throw new Error("prospective recovery split path escapes its root");
      const receiptOwned = new Set([...split.expectedFiles, resolve(participant.configPath), resolve(participant.receiptPath)]);
      if (split.additions.some((path) => receiptOwned.has(path))) throw new Error("prospective recovery addition overlaps receipt ownership");
      const expectedPaths = [...receiptOwned, ...split.additions].sort();
      const paths = participant.paths.map((path) => resolve(path)).sort();
      if (new Set(paths).size !== paths.length || paths.length !== expectedPaths.length || paths.some((path, index) => path !== expectedPaths[index])) throw new Error("prospective recovery paths do not exactly match expectedFiles and additions");
    }
    return;
  }
  ...
  const root = resolve(participant.root);
  const additions = participantAdditions(participant);
  if (additions.some((path) => !isContained(root, path))) throw new Error("existing participant addition escapes its root");
  if (resolve(participant.receipt.root) !== root || !Array.isArray(participant.receipt.files) || participant.receipt.files.some((path) => typeof path !== "string")) throw new Error("existing participant receipt authority is invalid");
  const receiptPaths = participant.receipt.files.map((path) => resolve(path));
  if (new Set(receiptPaths).size !== receiptPaths.length) throw new Error("existing participant receipt authority is invalid");
  const receiptOwned = new Set([...receiptPaths, resolve(participant.configPath), resolve(participant.receiptPath)]);
  if (additions.some((path) => receiptOwned.has(path))) throw new Error("existing participant addition overlaps receipt ownership");
  const expectedPaths = [...receiptOwned, ...additions].sort();
  const paths = participant.paths.map((path) => resolve(path)).sort();
  if (new Set(paths).size !== paths.length || paths.length !== expectedPaths.length || paths.some((path, index) => path !== expectedPaths[index])) throw new Error("existing participant paths do not exactly match receipt ownership and additions");
  let snapshottedReceipt;
  try { snapshottedReceipt = JSON.parse(Buffer.from(participant.receiptSnapshot.data, "base64").toString("utf8")); }
  catch { throw new Error("existing participant receipt snapshot is invalid"); }
  if (JSON.stringify(snapshottedReceipt) !== JSON.stringify(participant.receipt)) throw new Error("existing participant receipt snapshot authority is invalid");
}
...
function assertParticipantSnapshots(participants, snapshots) {
  for (const participant of participants) {
    ...
    } else if (participant.role === "existing-installation-target") {
      if (!sameSnapshot(participant.receiptSnapshot, snapshots[participant.receiptPath])) {
        throw new TransactionLockError("recovery_required", `existing transaction receipt snapshot changed: ${participant.receiptPath}`);
      }
      for (const path of participant.additions) {
        if (snapshots[path]?.state !== "absent") throw new TransactionLockError("recovery_required", `existing transaction addition preimage is not absent: ${path}`);
      }
    }
  }
}
...
function assertRecoveryManifestAuthorized(manifest, authority) {
  ...
  for (const participant of participants) {
    ...
    if (participant.role === "existing-installation-target") {
      const authorizedInstallation = authority.participants.some((candidate) =>
        resolve(candidate.root) === participantRoot
        && resolve(candidate.coordinationRoot ?? candidate.root) === coordinationRoot
        && resolve(candidate.configPath) === participant.configPath
        && resolve(candidate.receiptPath) === participant.receiptPath
        && JSON.stringify(candidate.paths) === JSON.stringify(participant.paths)
        && (
          (
            candidate.role === "prospective-installation-target"
            && Array.isArray(candidate.expectedFiles)
            && Array.isArray(candidate.additions)
            && JSON.stringify(candidate.expectedFiles) === JSON.stringify(participant.receipt.files.map((path) => resolve(path)).sort())
            && JSON.stringify(candidate.additions) === JSON.stringify(participant.additions)
          )
          || (
            candidate.role === "existing-installation-target"
            && JSON.stringify(participantAdditions(candidate)) === JSON.stringify(participant.additions)
            && JSON.stringify(candidate.receipt) === JSON.stringify(participant.receipt)
            && sameSnapshot(candidate.receiptSnapshot, participant.receiptSnapshot)
          )
        )
      );
      if (!authorizedInstallation) throw new TransactionLockError("recovery_required", "transaction recovery existing participant does not exactly match caller authority");
    }
  }
