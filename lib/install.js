...
export async function install({ scope, projectRoot, cwd = process.cwd(), env = process.env, transactionApi } = {}) {
  ...
  const legacyExpectedFiles = legacyReceiptOwnedPaths(layout, expectedFiles);
  await establishInstallationCoordination(transactions, layout, false);
  await recoverInstallationVariants(transactions, layout, expectedFiles, legacyExpectedFiles);
  const existingTarget = await pathExists(receiptPath)
    ? await existingInstallationTargetForUpgrade(layout, receiptPath, expectedFiles, legacyExpectedFiles)
    : null;
  ...
  const leader = installLeaderConfig(existingConfig, desiredMatrix.leader, layout.configPath, oldReceipt);
  const configWithManagedBlock = replaceManagedBlock(leader.config, block, layout.configPath);
  ...
  const receipt = {
    ...
    leaderConfig: leader.state,
    setupAgentMatrix: setupReceiptMatrix(desiredMatrix)
  };
  if (feature.state) receipt.featureConfig = feature.state;
  const legacyVerifier = legacyExpectedFiles.find((path) => basename(path) === `${LEGACY_VERIFIER_NAME}.toml`);
  const removals = existingTarget?.paths.includes(resolve(legacyVerifier)) ? [legacyVerifier] : [];
...
export async function uninstall({ projectRoot, cwd = process.cwd(), env = process.env, transactionApi } = {}) {
  ...
  const projectLegacyFiles = legacyReceiptOwnedPaths(project, projectExpectedFiles);
  const globalLegacyFiles = legacyReceiptOwnedPaths(global, globalExpectedFiles);
  await recoverExistingControlStoreVariants(transactions, project, projectExpectedFiles, projectLegacyFiles);
  const projectReceipt = await pathExists(join(project.configRoot, RECEIPT));
  if (!projectReceipt) {
    await rejectUnmanagedProjectCollision(project);
    await recoverExistingControlStoreVariants(transactions, global, globalExpectedFiles, globalLegacyFiles);
  }
  ...
  const legacyExpectedFiles = candidate === project ? projectLegacyFiles : globalLegacyFiles;
...
function legacyReceiptOwnedPaths(layout, expectedFiles) {
  return [...new Set([
    ...expectedFiles.map((path) => resolve(path)),
    resolve(join(layout.agentsRoot, `${LEGACY_VERIFIER_NAME}.toml`))
  ])].sort();
}

async function existingInstallationTargetForUpgrade(layout, receiptPath, expectedFiles, legacyExpectedFiles) {
  try {
    ...
    if (!/receipt does not match the installed package paths/.test(currentError.message)) throw currentError;
    try {
      return await existingInstallationTarget({
        root: layout.root,
        configPath: layout.configPath,
        receiptPath,
        expectedFiles: legacyExpectedFiles
      });
    } catch (legacyError) {
      if (/receipt does not match the installed package paths/.test(legacyError.message)) throw currentError;
      throw legacyError;
    }
  }
}
...
export function readLeaderConfig(content, configPath = "config.toml") {
  validateMarkerPair(configPath, content, LEADER_MANAGED_START, LEADER_MANAGED_END, "csx leader defaults");
  if (!content.includes(LEADER_MANAGED_START)) {
    throw new Error(`managed leader defaults are missing in ${configPath}`);
  }
  const region = managedRegion(content, LEADER_MANAGED_START, LEADER_MANAGED_END);
  const document = scanToml(region.text, configPath);
  const pair = {};
  for (const key of ["model", "model_reasoning_effort"]) {
    const matches = document.assignments.filter(({ table, key: path }) =>
      table.length === 0 && samePath(path, [key])
    );
    if (matches.length !== 1) throw new Error(`managed leader defaults are invalid in ${configPath}`);
    pair[key === "model" ? "model" : "reasoning"] = decodeTomlString(matches[0].value, configPath);
  }
  return pair;
}

export function updateLeaderConfig(content, pair, configPath = "config.toml") {
  readLeaderConfig(content, configPath);
  const region = managedRegion(content, LEADER_MANAGED_START, LEADER_MANAGED_END);
  const replacement = `${leaderBlock(pair)}\n`;
  return `${content.slice(0, region.lineStart)}${replacement}${content.slice(region.lineEnd)}`;
}

function installLeaderConfig(content, pair, configPath, receipt) {
  const hasMarker = content.includes(LEADER_MANAGED_START);
  if (hasMarker) {
    if (!receipt?.leaderConfig) {
      throw new Error(`managed leader defaults have no matching receipt metadata in ${configPath}`);
    }
    validateLeaderReceipt(receipt.leaderConfig, configPath);
    return {
      config: updateLeaderConfig(content, pair, configPath),
      state: receipt.leaderConfig
    };
  }
  if (receipt?.leaderConfig) {
    throw new Error(`managed leader defaults are missing in ${configPath}`);
  ...
  const removals = [];
  for (const key of ["model", "model_reasoning_effort"]) {
    const assignments = document.assignments.filter(({ table, key: path }) =>
      table.length === 0 && samePath(path, [key])
    );
    if (assignments.length > 1) throw new Error(`duplicate top-level TOML key in ${configPath}`);
    const assignment = assignments[0];
    if (assignment) decodeTomlString(assignment.value, configPath);
    originals[key] = assignment?.line ?? null;
    if (assignment) removals.push(assignment);
  }
  let without = content;
  for (const assignment of removals.sort((left, right) => right.start - left.start)) {
    without = replaceSlice(without, assignment.start, assignment.end, "");
  }
  const insertionCandidates = [
    scanToml(without, configPath).headers[0]?.start,
    without.indexOf(MANAGED_START),
    without.indexOf(FEATURE_MANAGED_START)
  ].filter((index) => Number.isInteger(index) && index >= 0);
  const insertion = insertionCandidates.length ? Math.min(...insertionCandidates) : without.length;
  return {
    config: insertBlock(without, insertion, leaderBlock(pair), true),
    state: { version: 1, originals }
  };
}
...
function leaderBlock({ model, reasoning }) {
  if (typeof model !== "string" || !model || typeof reasoning !== "string" || !reasoning) {
    throw new Error("leader model configuration is invalid.");
  }
  return [
...
async function recoverInstallationVariants(transactions, layout, expectedFiles, legacyExpectedFiles) {
  try {
    await recoverInstallationTransactions(
      transactions,
      layout,
      installationRecoveryAuthority(layout, expectedFiles)
    );
  } catch (error) {
    ...
    await recoverInstallationTransactions(
      transactions,
      layout,
      installationRecoveryAuthority(layout, legacyExpectedFiles)
    );
  }
}
...
async function recoverExistingControlStoreVariants(transactions, layout, expectedFiles, legacyExpectedFiles) {
  try {
    await recoverExistingControlStore(
      transactions,
      layout,
      installationRecoveryAuthority(layout, expectedFiles)
    );
