...
export async function install({ scope, projectRoot, cwd = process.cwd(), env = process.env, transactionApi } = {}) {
  ...
  const legacyVerifier = legacyVerifierPath(layout);
...
export async function uninstall({ projectRoot, cwd = process.cwd(), env = process.env, transactionApi } = {}) {
  ...
  await recoverExistingControlStoreVariants(transactions, project, projectRecoveryCandidates);
  const projectReceipt = await pathExists(join(project.configRoot, RECEIPT));
  if (!projectReceipt) {
    await rejectUnmanagedProjectCollision(project);
    await recoverExistingControlStoreVariants(transactions, global, globalRecoveryCandidates);
  }
...
function exactReceiptCandidates(layout, expectedFiles, { includeUpgradeAdditions = true } = {}) {
  ...
  const preLoop = current.filter((path) => !loopSet.has(path));
  ...
  const candidates = [
    ...
    candidate("pre-loop", preLoop),
    candidate("current-verifier-legacy", [...current, verifier]),
    candidate("pre-loop-verifier-legacy", [...preLoop, verifier])
  ];
  for (const entry of candidates) {
    const expectedAdditionCount = entry.name.startsWith("pre-loop") && includeUpgradeAdditions ? 2 : 0;
