...
async function looksLikeCanonicalReceipt(root, path) {
  ...
  try {
    ...
    const legacy = legacyReceiptOwnedPaths(projectLayout(root), expected);
    return [expected, legacy].some((candidate) =>
      received.length === candidate.length &&
      received.every((file, index) => file === candidate[index])
    ) || (
...
async function recoverInstallationTransactions(transactions, layout, authority) {
  ...
  try {
    ...
    if (!isRetryableRecoveryAuthorityError(error)
...
async function recoverInstallationVariants(transactions, layout, expectedFiles, legacyExpectedFiles) {
  try {
    ...
    return recoverInstallationTransactions(
      transactions,
      layout,
...
async function recoverExistingControlStoreVariants(transactions, layout, expectedFiles, legacyExpectedFiles) {
  try {
    ...
    if (!isRetryableRecoveryAuthorityError(error)) throw error;
    return recoverExistingControlStore(
      transactions,
      layout,
      installationRecoveryAuthority(layout, legacyExpectedFiles)
    );
