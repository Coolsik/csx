...
If a required csx role is missing, ask the user to run `$csx:setup`. For a trivial clarification only, continue with explicit `Assumptions` and `Open Questions` and label the artifact `DEGRADED: independent analysis unavailable`.
