...
If either required role is unavailable for a substantial review, ask the user to run `$csx:setup`; do not report `APPROVE`. Report `COMMENT` with `required csx role unavailable`.
