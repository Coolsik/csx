...
2. Confirm `csx-explorer` is available. If it is missing, ask the user to rerun `csx install` for the intended scope. Continue only for a trivial one-file lookup and label it `DEGRADED: custom agent unavailable`.
